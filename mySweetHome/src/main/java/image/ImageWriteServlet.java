...
public class ImageWriteServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String uploadPath = "/upload"; //파일을 저장할 폴더 이름
		int maxLimit = 5*1024*1024; //업로드 최대 크기 설정(5M)
		String encType = "UTF-8"; //언어코드
		String fileName = ""; //업로드가 성공하면 파일명을 저장할 변수
		//서블릿에서 업로드될 폴더를 찾아야한다.
		//이때 필요한 객체가 ServletContext
		ServletContext ctx = this.getServletContext();
		String realPath = ctx.getRealPath(uploadPath);
		System.out.println("upload 폴더의 절대경로 : "+realPath);
		String result =""; //게시글 등록 성공여부를 위한 변수 선언
		try{
			MultipartRequest multipart = new MultipartRequest(request, realPath, maxLimit, encType, new DefaultFileRenamePolicy());
			...
			if(fileName.equals("")) { //업로드 실패
				...
				boolean r = dao.putImageBBS(bbs); //insert 실행
				if(r == false) result = "N";
			}
		}catch(Exception e) {
			result = "N";
		...
		response.sendRedirect("uploadResult.jsp?R="+result);
	}
}
